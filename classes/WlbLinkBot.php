...
if ( !class_exists( 'classLink_Bot' ) ) {
	...
	class classLink_Bot {
		...
		public static function generate_links() {
			...
			$link_array[ 'home_pg' ][ 'normal_link' ] = self::link_a_rule( $home_url, null );
			$link_array[ 'home_pg' ][ '404' ] = self::link_a_rule( $home_url . '/zyxwvutsr10up', null );
			$link_array[ 'blog_pg' ][ 'normal_link' ] = ($blog_page) ? self::link_a_rule( get_permalink( $blog_page ), 'page' ) : '--';

			if ( $blog_view[ 'no_of_pages' ] > 1 ) {
				$link_array[ 'blog_pg' ][ 'paginated_link' ] = ($blog_page) ? self::link_a_rule( self::get_paginated_link( $blog_page, 2 ), 'page' ) : '--';
			}
			$link_array[ 'blog_pg' ][ 'ex_pgl' ] = ($blog_page) ? self::link_a_rule( self::get_paginated_link( $blog_page, $blog_view[ 'no_of_pages' ] + 7 ), 'page' ) : '--';
			//search page
			$link_array[ 'search' ][ 'results' ] = add_query_arg( 's', 'a', $home_url );

			if ( $blog_view[ 'no_of_pages' ] > 1 ) {
				$link_array[ 'search' ][ 'paginated_results' ] = add_query_arg( 's', 'a', self::search_pagination( 2 ) );
			}
			...
			if ( !empty( $template_pages ) ) {
				foreach ( $template_pages as $type => $data ) {
					$link_array[ $type ][ 'normal_link' ] = isset( $data[ 'no_pagination' ] ) ? self::link_a_rule( get_permalink( $data[ 'no_pagination' ] ), $type ) : '--';
					$link_array[ $type ][ 'paginated_link' ] = isset( $data[ 'pagination' ] ) ? self::link_a_rule( self::get_paginated_link( $data[ 'pagination' ][ 'id' ], 2 ), $type ) : '--';
					$link_array[ $type ][ 'pagination_exceed' ] = isset( $data[ 'pagination' ] ) ? self::link_a_rule( self::get_paginated_link( $data[ 'pagination' ][ 'id' ], ((int) ($data[ 'pagination' ][ 'pages_no' ] + 7) ) ), $type ) : '--';
					if ( $data[ 'comments' ] ) {
						$link_array[ $type ][ 'comments_link' ] = isset( $data[ 'no_pagi_com' ] ) ? self::link_a_rule( get_permalink( $data[ 'no_pagi_com' ] ), $type ) : '--';
						$link_array[ $type ][ 'comments_pagi_link' ] = isset( $data[ 'paginated_com' ] ) ? self::link_a_rule( get_permalink( $data[ 'paginated_com' ][ 'id' ] ), $type ) : '--';
						$link_array[ $type ][ 'com_pagination_exceed' ] = isset( $data[ 'paginated_com' ] ) ? self::link_a_rule( self::get_comment_pagenum_link( $data[ 'paginated_com' ][ 'id' ], ((int) ($data[ 'paginated_com' ][ 'pages_no' ] + 7) ) ), $type ) : '--';
					}
				}
			}
			// post links
				...
				foreach ( $terms as $term ) {
					...
					if ( $term->parent ) {
						if ( $no_of_pages == 1 && $no_pagi_term_p == false ) {
							$link_array[ $taxonomy ][ 'normal_link_parent' ] = self::link_a_rule( get_term_link( $term ), $taxonomy );
							$no_pagi_term_p = true;
							$term_usecases++;
						} elseif ( $no_of_pages > 1 && $pagi_term_p == false ) {
							$link_array[ $taxonomy ][ 'paginated_link_parent' ] = self::link_a_rule( self::term_pagination( $term, 2 ), null );
							$link_array[ $taxonomy ][ 'pagination_exceed_parent' ] = self::link_a_rule( self::term_pagination( $term, $blog_view[ 'no_of_pages' ] + 7 ), null );
							$pagi_term_p = true;
							$term_usecases++;
						}
					} else {
						if ( $no_of_pages == 1 && $no_pagi_term == false ) {
							$link_array[ $taxonomy ][ 'normal_link' ] = self::link_a_rule( get_term_link( $term ), $taxonomy );
							$no_pagi_term = true;
							$term_usecases++;
						} elseif ( $no_of_pages > 1 && $pagi_term == false ) {
							$link_array[ $taxonomy ][ 'paginated_link' ] = self::link_a_rule( self::term_pagination( $term, 2 ), null );
							$link_array[ $taxonomy ][ 'pagination_exceed' ] = self::link_a_rule( self::term_pagination( $term, $blog_view[ 'no_of_pages' ] + 7 ), null );
							$pagi_term = true;
							$term_usecases++;
						}
					}
			...
			//Year archive links
			$link_array[ 'year_archive' ][ 'normal_link' ] = self::link_a_rule( get_year_link( date( 'Y' ) ), null );
			$year_posts = self::count_post_by_date( date( "Y-m-d", strtotime( "-1 year", time() ) ) );
			$year_pages = ceil( $year_posts / $blog_view[ 'posts_per_page' ] );

			if ( $year_pages > 1 ) {
				$link_array[ 'year_archive' ][ 'paginated_link' ] = self::link_a_rule( self::date_archive_pagination( 'year', 2 ), null );
				$link_array[ 'year_archive' ][ 'pagination_exceed' ] = self::link_a_rule( self::date_archive_pagination( 'year', $year_pages + 7 ), null );
			}
			//Month archive links
			$link_array[ 'month_archive' ][ 'normal_link' ] = self::link_a_rule( get_month_link( date( 'Y' ), date( 'm' ) ), null );
			...
			if ( $month_pages > 1 ) {
				$link_array[ 'month_archive' ][ 'paginated_link' ] = self::link_a_rule( self::date_archive_pagination( 'month', 2 ), null );
				$link_array[ 'month_archive' ][ 'pagination_exceed' ] = self::link_a_rule( self::date_archive_pagination( 'month', $month_pages + 7 ), null );
			}
			//Day Posts
			$link_array[ 'day_archive' ][ 'normal_link' ] = self::link_a_rule( get_day_link( date( 'Y' ), date( 'm' ), date( 'd' ) ), null );
			$day_posts = self::count_post_by_date( date( "Y-m-d" ) );
			$day_pages = ceil( $day_posts / $blog_view[ 'posts_per_page' ] );

			if ( $day_pages > 1 ) {
				$link_array[ 'day_archive' ][ 'paginated_link' ] = self::link_a_rule( self::date_archive_pagination( 'day', 2 ), null );
				$link_array[ 'day_archive' ][ 'pagination_exceed' ] = self::link_a_rule( self::date_archive_pagination( 'day', $day_pages + 7 ), null );
			}
			return $link_array;
		}
		...
		public static function get_template_pages() {
			...
			$max_pg_comments = get_option( 'comments_per_page' );
			foreach ( $template_pages as $spage ) {
